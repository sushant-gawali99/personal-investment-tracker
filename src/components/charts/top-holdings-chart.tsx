...
export function TopHoldingsChart({ holdings }: { holdings: Holding[] }) {
  ...
  if (!top.length) {
    return <div className="flex items-center justify-center h-48 text-[var(--text-secondary)] text-sm">No holdings</div>;
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={top} layout="vertical" margin={{ top: 0, right: 16, left: 0, bottom: 0 }}>
        ...
        <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: "var(--text-primary)" }} tickLine={false} axisLine={false} width={74} />
        ...
        <Bar dataKey="value" radius={[0, 6, 6, 0]} animationDuration={600}>
          {top.map((entry, i) => (
            <Cell key={i} fill={entry.gain ? "var(--accent-success)" : "var(--accent-error)"} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
