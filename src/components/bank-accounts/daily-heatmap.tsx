  ...
  return (
    <div className="ab-card p-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="text-[16px] font-semibold text-[var(--text-primary)] tracking-tight">Daily Spending</h3>
      ...
      <div className="grid grid-cols-7 gap-1.5 mb-3">
        {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
          <div key={i} className="text-[10px] text-[var(--text-tertiary)] text-center font-semibold tracking-wider">
            {d}
          </div>
        ))}
        {cells.map((c, i) => {
          ...
          return (
            <button
              ...
              className="aspect-square rounded-[6px] text-[10px] font-semibold flex items-center justify-center transition-all hover:scale-110 hover:ring-2 hover:ring-white/20 focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
              style={{
                background: intensityColor(c.value),
                color: empty ? "var(--text-tertiary)" : c.value / max > 0.4 ? "#fff" : "var(--text-primary)",
      ...
      {/* Intensity legend */}
      <div className="flex items-center gap-2 mt-4 text-[10px] text-[var(--text-tertiary)]">
        <span>Less</span>
        {[0, 0.2, 0.45, 0.7, 1].map((t, i) => (
          <span
            key={i}
            className="w-4 h-4 rounded-[4px]"
            style={{ background: t === 0 ? "var(--surface-muted)" : `rgba(255,56,92,${0.15 + 0.75 * t})` }}
          />
        ))}
        <span>More</span>
      </div>
