...
export async function extractTransactions(
  ...
  if (preExtractedText !== undefined) {
    // Excel (or other pre-converted) path: text was already extracted by the caller.
    // Skip PDF parsing entirely — there is no PDF to parse.
    extractedText = preExtractedText.trim().length > 0 ? preExtractedText : null;
    console.log(`[extract] pre-extracted text provided, len=${preExtractedText.length}, skipping pdf-parse`);
  } else {
    const tPdf = Date.now();
    try {
