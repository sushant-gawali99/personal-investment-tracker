...
function FDRow({ fd }: { fd: FDEntry }) {
  ...
  return (
    <div className="border border-[#2a2a2d] rounded-lg overflow-hidden">
      ...
      >
        {expanded ? <ChevronDown size={14} className="text-[#606065] shrink-0" /> : <ChevronRight size={14} className="text-[#606065] shrink-0" />}
        ...
        {fd.linkedFdId && (
          <Link
            ...
            className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full border border-[rgba(255,56,92,0.35)] text-[#ff385c] bg-[rgba(255,56,92,0.08)] hover:bg-[rgba(255,56,92,0.15)] transition-colors shrink-0"
          >
            <Link2 size={10} />
            In system
          </Link>
...
export function StatementReportForm() {
  ...
  const extract = async () => {
    ...
    try {
      const res = await fetch("/api/fd/statement-report/extract", { method: "POST", body: form });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error ?? "Extraction failed");
      setReportData(json.data as FDReportData);
      setStep("preview");
    } catch (err) {
      ...
      <div className="space-y-5">
        ...
        <div className="bg-[#0d0d0f] border border-[#2a2a2d] rounded-xl p-5 space-y-3">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="text-[16px] font-bold text-[#ededed]">{reportData.bankName}</p>
              {reportData.accountHolderName && (
                <p className="text-[13px] text-[#a0a0a5] mt-0.5">{reportData.accountHolderName}</p>
              )}
              {reportData.accountNumber && (
                <p className="text-[12px] text-[#606065] mt-0.5">A/C {reportData.accountNumber}</p>
              )}
            </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        onClick={extract}
        disabled={!file}
        className="px-5 py-2 rounded-full bg-[rgba(255,56,92,0.15)] border border-[rgba(255,56,92,0.35)] text-[#ff385c] text-[13px] font-bold hover:bg-[rgba(255,56,92,0.22)] hover:border-[rgba(255,56,92,0.5)] transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
      >
        <FileText size={14} />
        Extract FD Report
      </button>
